...
const VideoPlayback = () => {
  ...
  // Improved FPS calculation using requestAnimationFrame for more accurate measurements
  const startFpsCalculation = () => {
    ...
    const calculateFrame = (timestamp) => {
      ...
      fpsAnimationFrameRef.current = requestAnimationFrame(calculateFrame);
    };
  ...
  const startWebcamRecording = async () => {
    try {
      // Specify explicit constraints for high frame rate recording
      const stream = await navigator.mediaDevices.getUserMedia({
        video: {
          width: { ideal: 1920 },
          height: { ideal: 1080 },
          frameRate: { ideal: 60, min: 30 }  // Request high frame rate
        },
        audio: true,
      });
      ...
      // Check if VP9 is supported for better quality
      ...
      if (!MediaRecorder.isTypeSupported(options.mimeType)) {
        ...
        if (!MediaRecorder.isTypeSupported(options.mimeType)) {
          ...
          if (!MediaRecorder.isTypeSupported(options.mimeType)) {
            options = {}; // Last resort: use browser defaults
          }
        }
      }
      ...
      // Use smaller timeslice for more frequent data chunks
  ...
  const uploadRecording = async (blob) => {
    ...
    try {
      ...
      // Log the video details before encryption
      const tempVideo = document.createElement('video');
      tempVideo.src = URL.createObjectURL(blob);
      
      tempVideo.onloadedmetadata = async () => {
        console.log("Original video metadata:");
        console.log("Duration:", tempVideo.duration);
        console.log("Video width:", tempVideo.videoWidth);
        console.log("Video height:", tempVideo.videoHeight);
        URL.revokeObjectURL(tempVideo.src);
        
        // Continue with upload after getting metadata
        await continueUpload(blob);
      };
      
      // If metadata loading fails, continue anyway
      tempVideo.onerror = async () => {
        console.error("Could not load video metadata");
        URL.revokeObjectURL(tempVideo.src);
        await continueUpload(blob);
      };
    } catch (error) {
  ...
  const continueUpload = async (blob) => {
    try {
      const videoAesKey = Array.from(crypto.getRandomValues(new Uint8Array(32)))
        .map((b) => b.toString(16).padStart(2, "0"))
        .join("");
      ...
      for (let pair of formData.entries()) {
        console.log(pair[0] + ': ' + (pair[0] === 'video' || pair[0] === 'video_encrypted_aes_key' ? '[BLOB DATA]' : pair[1]));
      }
  ...
  return (
    <div className="bg-[#1A0C25] min-h-screen flex flex-col justify-center items-center">
      ...
      <div className="absolute top-4 right-4 z-20 flex items-center space-x-2 bg-black bg-opacity-50 px-4 py-2 rounded-full">
        ...
        ></div>
        <span className="text-white text-sm">
          {isRecording ? `Recording (${fps} FPS)` : "Not Recording"}
        </span>
      </div>
      {isLoading && (
        <div
