...
const PricingCard = ({ discount, price, originalPrice, validity, sessions, savings, background, isPopular }) => (
  <div className={`w-[320px] h-[400px] rounded-3xl p-8 flex flex-col justify-between items-center text-white relative ${background}`}>
    {isPopular && (
      <img src={popularImage} alt="Popular" className="absolute top-0 left-0 w-17 h-16" />
    )}
    ...
    <Link to="/payment" className="px-2 w-auto md:h-[2.5rem] text-[#F6E8FB] py-4 rounded-full text-lg font-semibold flex items-center space-x-2 hover:bg-[#B7407D] transition-colors border border-[#9C00AD]">
